...
If guihicon>0 Then sendMessage(hWin,WM_SETICON,NULL,Cast(LPARAM,guihIcon))
...
Function filedialog(ByRef titre As String,ByRef filtre As String = "",ByRef file0 As String="") As String
...
Sub guiinitopenglfont0
   ...
   guiopenglfontlist = 99000' glGenLists(256)
	wglUseFontOutlines(guiopenglwindc, 0,256, guiopenglfontlist, 0.0f, 0.0f, WGL_FONT_POLYGONS,0)
	'wglUseFontOutlines(guiopenglwindc, 0,256, guiopenglfontlist, 0.0f, 0.1f, WGL_FONT_POLYGONS,0)
	'wglUseFontOutlines(guiopenglwindc, 0,256, guiopenglfontlist, 0.0f, 0.0f, WGL_FONT_LINES,0)
	DeleteObject(hFont)
End Sub
...
If maxcolor<>255 Then 
  ...
  If typealpha=0 Then  
   ...
   Next
  Else
   For i=1 To UBound(picbits) ' add alpha if color<maxcolor
	  'pix=picbits(i)
	  'If ((pix And 255)+((pix Shr 8)And 255)+((pix Shr 16)And 255))>3*maxcolor Then 
	  pixptr=Cast(UByte Ptr,@picbits(i))
	  If (pixptr[0]+pixptr[1]+pixptr[2])<maxcolor3 Then 
...
Return itexture 
End Function
Dim Shared As uint guifonttexture
Sub guiinitopenglfont
Dim As Integer i,j,k,ii
Dim As Single x1,x0,y,tx,ty,tx1,ty1
guifonttexture=guiloadtexture("gui_font.bmp",200,127,1)
'guifonttexture=guiloadtexture("gui_font.jpg",200,127,1)
guiopenglfontlist = 99000' glGenLists(256)
For i=0 To 255
	ii=0
	j=(i Mod 16)*16
	k=(i\16)*16
   glnewlist guiopenglfontlist+i+1,GL_COMPILE 'GL_COMPILE_AND_EXECUTE'4865
   glEnable GL_TEXTURE_2D 
   glenable gl_alpha_test
   glAlphaFunc(gl_less,10/254)
   glBindTexture GL_TEXTURE_2D,guifonttexture
   x1=0.7:x0=0:y=1.0
   tx=j/256:ty1=1-k/256:tx1=(j+8)/256:ty=1-(k+12)/256
   'tx=0:ty=0:tx1=1:ty1=1
	glbegin(gl_quads)
	glTexCoord2f(tx,ty)
	glvertex3f(x0,0,0)
	gltexcoord2f(tx1,ty)
...
Dim As Integer k
Dim As Single tx,ty,dtx=1/16,dx0=0,dx=0.5,dty=0.7/16
'Dim As Single tx,ty,dtx=0.99*1/16,dx0=0,dx=0.8,dty=0.99*0.7/16
k=Asc(car)
tx=(k\16)/16-1/32:ty=1-(k Mod 16)/16-1/32
