...
twiki=1
If Not Eof(file) Then Line Input #file,ficin:twiki=Val(ficin)
Close #file
Sub mysubquit
	quit=1
End Sub
...
button("win.wiki","wiki",@subwiki,250,5,60,20)
...
loadSynonyms()
'loadSynonymsprolog():saveSynonyms()
loadvars()
subloaddata2(0)
setformatconst()
...
While quit=0 And Not guitestKey(vk_escape)
  ...
  If guitestkey(vk_r) And guitestkey(vk_control) Then
  	  confirm("reset aiml vars ?","confirm",resp)
  	  If resp="yes" Then resetvars()
  EndIf
...
Sub drawtexture2(opt As Integer=0)
   ...
   glBindTexture GL_TEXTURE_2D,persotext 
      ...
      Var bra0=(ibright0)/100.0
      Var bra1=(ibright1)/100.0
      kr*=bra0:kg*=bra0:kb*=bra0
      kr1*=bra1:kg1*=bra1:kb1*=bra1
  ...
  EndIf 
   ...
   'gllightfv(gl_light0,GL_ambient,glparam4f(0.1,0.1,0.1,1)) 'GL_AMBIENT GL_DIFFUSE GL_SPECULAR GL_POSITION
      ...
      EndIf   
		...
		If auxtext<>"" Then gldrawtext("auxtext="+Str(auxtext),50,560,1)			
		If Mid(auxtext,70)<>"" Then gldrawtext("auxtext="+mid(auxtext,70),50,580,1)						
		If Mid(auxtext,140)<>"" Then gldrawtext("auxtext="+Right(auxtext,70),50,600,1)						
...
cr=Chr(13)
	...
	msg+=cr+"ctrl+R or say 'reset vars' => reset vars"
	guinotice msg	
End Sub
